...
describe 'tpm' do
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'with detected TPM and enable_pkcs_interface => true' do
        let(:facts) do
          os_facts.merge({ :has_tpm => true })
        end
        let(:params) {{ :enable_pkcs_interface => true }}
        ...
        it { is_expected.to contain_package('tpm-tools').with_ensure('latest') }
        it { is_expected.to contain_package('trousers').with_ensure('latest') }
        it { is_expected.to contain_package('tpm-tools-pkcs11').with_ensure('latest') }
        it { is_expected.to contain_service('tcsd').with({
          'ensure'  => 'running',
          'enable'  => true,
        }) }
